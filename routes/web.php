...
use App\Http\Controllers\Panel\ClientController as PanelClientController;
...
use App\Http\Controllers\Panel\PostCategoryController as PanelPostCategoryController;
...
use App\Http\Controllers\Panel\PanelRoleController as PanelRoleController;
use App\Http\Controllers\Panel\SiteSettingController as PanelSiteSettingController;
...
Route::middleware(['auth', 'panel'])
    ...
    ->group(function (): void {
        ...
        Route::resource('post-categories', PanelPostCategoryController::class)->except('show')->middleware('panel.permission:post_categories.manage');
        Route::resource('posts', PanelPostController::class)->except('show')->middleware('panel.permission:posts.manage');
        Route::resource('clients', PanelClientController::class)->except('show')->middleware('panel.permission:clients.manage');
        Route::resource('quote-requests', PanelQuoteRequestController::class)->only(['index', 'show', 'update', 'destroy'])->middleware('panel.permission:quote_requests.manage');
        Route::resource('product-categories', PanelProductCategoryController::class)->except('show')->middleware('panel.permission:product_categories.manage');
        Route::resource('products', PanelProductController::class)->except('show')->middleware('panel.permission:products.manage');
        Route::get('settings', [PanelSiteSettingController::class, 'edit'])->middleware('panel.permission:settings.manage')->name('settings.edit');
        Route::put('settings', [PanelSiteSettingController::class, 'update'])->middleware('panel.permission:settings.manage')->name('settings.update');
    });
